...
// yanting
//  judge whether the cast is 1) related to function constant
//  2) between function pointer
//  store them seperately into 2 sets
bool TyPM::judgeCast(User *CO, Module* M)
{
    
    Value *fromU = CO->getOperand(0);
    Value *toU = CO;
    ...
    else if (auto *PT = dyn_cast<PointerType>(fromU->getType()))
    {
        ...
        else if (auto *PT = dyn_cast<PointerType>(toU->getType()))
        {
            if (isa<FunctionType>(PT->getElementType()))
            {
                // llvm::outs() << "GOT FPTR CAST CASE:  " << *CO;
...
void TyPM::printBCs(set<User *> &CastSet)
{
...
void TyPM::processCasts(set<User *> &CastSet, Module *M)
{
    ...
    for (auto CO : CastSet)
    {
        ...
        if (!isTargetTy(ETyFrom) && !isContainerTy(ETyFrom) && !isTargetTy(ETyTo) && !isContainerTy(ETyTo))
        {
            continue;
        }
...
void TyPM::parseTargetTypesInCalls(CallInst *CI, Function *CF)
{
         ...
         AI != E; ++AI)
    {
        ...
        else
        {
            ...
            if (CI->isIndirectCall())
            {
                if (ParsedModuleTypeICallMap[MP].find(ATy) != ParsedModuleTypeICallMap[MP].end())
                    continue;
                ParsedModuleTypeICallMap[MP].insert(ATy);
            }
            else
            {
                if (ParsedModuleTypeDCallMap[MP].find(ATy) != ParsedModuleTypeDCallMap[MP].end())
                    continue;
    ...
    else
    {
        ...
        if (CI->isIndirectCall())
        {
            if (ParsedModuleTypeICallMap[MP].find(RTy) != ParsedModuleTypeICallMap[MP].end())
                return;
            ParsedModuleTypeICallMap[MP].insert(RTy);
        }
        else
        {
            if (ParsedModuleTypeDCallMap[MP].find(RTy) != ParsedModuleTypeDCallMap[MP].end())
                return;
                                  ...
                                  set<Type *> &TargetTypes, Module *M)
{
    ...
    if (ParsedTypeMap.find(make_pair(M, VTy)) != ParsedTypeMap.end())
    {
                                ...
                                set<Module *> &MSet)
{
    ...
    for (auto TyIdx : TyList)
    {
        ...
        //
        // Externality check
        if (storedTypeIdxMap[M].find(TyIdx.first) != storedTypeIdxMap[M].end())
        {
            if ((storedTypeIdxMap[M][TyIdx.first].find(TyIdx.second) != storedTypeIdxMap[M][TyIdx.first].end()) || TyIdx.second == -1)
                break;
        }
        Outermost = TyIdx;
        break;
    ...
    if (ResolvedDepModulesMap.find(TyM) != ResolvedDepModulesMap.end())
    ...
    for (auto CI : ICallSet)
    {
        ...
        for (auto Callee : Ctx->Callees[CI])
        {
            ...
            else
            {
                ...
                if ((OutScopeFuncNames.find(FN) == OutScopeFuncNames.end())
...
bool TyPM::resolveStructTargets()
{
    ...
    for (auto SI : StoreInstSet)
    {
        ...
        if ((PO->getType() != Int8PtrTy[SI->getModule()]) && !criticalType)
            continue;
